...
The MCP server is read-only by design. It exposes Gateway status, doctor, health report, backend list, observability, session drilldown, governance insights, policy, policy exception, incident, evidence, provider risk, and control mapping tools. `gateway_session`, `gateway_session_spans`, and `gateway_session_drilldown` let an MCP-capable agent inspect a recorded session by ID without opening the web UI. `gateway_governance_insights` summarizes risk, behavior trends, compliance coverage, and admin action queue signals. `gateway_evidence_bundle` composes session detail, spans, model calls, policy decisions, evidence, incidents, provider risks, control mappings, and governance policies into one audit context bundle.

The MCP server also exposes attachable JSON resources such as `gateway://health-report`, `gateway://governance/insights`, `gateway://sessions/{session_id}`, `gateway://sessions/{session_id}/spans`, `gateway://governance/evidence`, and `gateway://evidence-bundles/session/{session_id}`. It does not expose backend creation, key generation, credential rotation, approvals, denials, or other mutation tools, and it redacts raw keys, tokens, authorization values, secrets, and encrypted credential material from tool and resource results.

Admins can call the same insights data directly:

```bash
curl "$MULTICA_API_URL/api/gateway/governance/insights" \
