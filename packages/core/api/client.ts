...
export class ApiClient {
  ...
  async getGatewayEvidenceBundle(params: GatewayEvidenceBundleParams): Promise<GatewayEvidenceBundleResponse> {
    ...
    const subjectCount = [sessionId, incidentId, policyDecisionId].filter(Boolean).length;
    if (subjectCount === 0) {
      throw new Error("one of session_id, incident_id, or policy_decision_id is required");
    }
    if (subjectCount > 1) {
      throw new Error("only one evidence bundle subject may be provided");
    }

    const search = new URLSearchParams();
    if (sessionId) search.set("session_id", sessionId);
    if (incidentId) search.set("incident_id", incidentId);
